...
void LeapData::setFingertipAngles() {
	...
	for (int i = 0; i < 5; i++) {
		if (extendedFingers[i] == 0) {
			...
			fingertipAngles.push_back(static_cast<float>((acos(b / (vect.getMagnitude(Point(0.f, 0.f, 0.f)) * handDirection.getMagnitude(Point(0.f, 0.f, 0.f))))) * 180.f / PI));
			count++;
		}
	}
}
...
void LeapData::setFingerAngles() {
	...
	for (size_t i = fingerAngles.size(); i < 4; ++i)
	{
...
void LeapData::parseRealTime(int lineNum, std::string line) {
	...
	switch (lineNum) {
	case 0:
		extendedFingers[0] = static_cast<int>(vect[0]);
		extendedFingers[1] = static_cast<int>(vect[1]);
		extendedFingers[2] = static_cast<int>(vect[2]);
		extendedFingers[3] = static_cast<int>(vect[3]);
		extendedFingers[4] = static_cast<int>(vect[4]);
	case 1:
